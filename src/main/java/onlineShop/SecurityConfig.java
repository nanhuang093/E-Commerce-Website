...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http
			.csrf().disable()
			.formLogin()
				.loginPage("/login")//use our own login page
				
			.and()// change back to http.
			.authorizeRequests()
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth
			...
			.usersByUsernameQuery("SELECT emailId, password, enabled FROM users WHERE emailId=?") //read user information in user class
